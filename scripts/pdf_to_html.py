...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output HTML path (default: <pdf>.html)",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        "--fragment",
        action="store_true",
        help="Output HTML fragment (no <html> wrapper)",
    )
...
def main(argv: list[str]) -> int:
    ...
    out_path = args.out if args.out is not None else pdf_path.with_suffix(".html")
    if not args.stdout:
    ...
    try:
        html = pdf_to_html(pdf_bytes, full_document=not args.fragment)
    except Exception as exc:  # noqa: BLE001
