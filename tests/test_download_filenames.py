...
def test_download_filename_from_upload_uses_pdf_stem() -> None:
    assert _download_filename_from_upload("test.pdf") == "test.md"
    assert _download_filename_from_upload(r"C:\tmp\foo.pdf") == "foo.md"
    assert _download_filename_from_upload("dir/subdir/bar.pdf") == "bar.md"
