...
- `RAGPREP_LIGHTON_PROFILE=ocr-fastest` (`balanced` / `ocr-fastest`)
...
- `RAGPREP_LIGHTON_FAST_POSTPROCESS_MODE=off` (`full` / `light` / `off`)

## OCR-Fastest Profile (OCR Required)

`RAGPREP_LIGHTON_PROFILE=ocr-fastest` is the current default profile.
It keeps OCR on every page and uses a speed-focused preset.
If you need previous tuning, set `RAGPREP_LIGHTON_PROFILE=balanced`.
...
To use previous balanced behavior:
...
- `RAGPREP_LIGHTON_PROFILE=ocr-fastest` (`balanced` or `ocr-fastest`)
